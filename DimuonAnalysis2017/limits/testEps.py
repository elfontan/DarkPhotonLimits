...
eff_file = TFile.Open("modifiedMllEff"+year+".root")
trig_teff = eff_file.Get("honemllD_clone")
trig_teff.Draw()
trig_teff.Paint("")
effgraph = trig_teff.GetPaintedGraph()
...
for d,m_fname in d_m:
    ...
    accCheck.append(acc)
    xSecCheck.append(xsec)

print limit1

c4=ROOT.TCanvas("c4","c4",700,500)
c4.SetLogy()
graph_xsec=ROOT.TGraph(len(mass),mass,xSecCheck)
graph_xsec.Draw("")
c4.SaveAs("XSecCheck.png")

c5=ROOT.TCanvas("c5","c5",700,500)
c5.SetLogy()
graph_acc=ROOT.TGraph(len(mass),mass,accCheck)
graph_acc.SetTitle("")
graph_acc.GetYaxis().SetTitle("Acceptance")
graph_acc.GetXaxis().SetTitle("m_{#mu#mu} [GeV]")
graph_acc.SetMarkerColor(kBlack)
graph_acc.Draw("")
latex2 = ROOT.TLatex(0.13,0.917,"#scale[1.1]{Signal Acceptance}")
latex2.SetNDC()
latex2.SetTextAlign(11)
latex2.Draw()
cmsTag5=ROOT.TLatex(0.45,0.917,"#bf{Drell-Yan Production}")
cmsTag5.SetNDC()
cmsTag5.SetTextAlign(11)
#cmsTag.SetTextFont(61)
cmsTag5.Draw()
c5.SaveAs("AcceptanceCheck.png")
