...
void MultiMakeCardsAndWS(){
  ...
  //INPUT FILE WITH HISTOGRAMS TO FIT BACKGROUND
	...
	//if (year == "2017") file=TFile::Open("/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/2017/ScoutingRunD/mergedHistos_v1.root");
        if (year[y] == "2017"){
          file=TFile::Open("/afs/cern.ch/work/w/wangz/public/darkphoton/sigma_p013/mergedHistos_mva_2017.root");       //38.7 fb -1 
          file2=TFile::Open("/afs/cern.ch/work/w/wangz/public/darkphoton/sigma_p013/mergedHistos_jpsi0p015_2017.root");//38.7 fb -1 
        }
        else if (year[y] == "2018"){
	...
	for(int i=0; i<400; i++){
	  ...
	  //if (i>139) continue;
		...
		newcardShape << Form("rate    		%f  		%f		\n",
				     effPT*effcuts*effgraph->Eval(mass,0,"S")*luminosity, catA->Integral());
